	...
	public void run()
	{
		try {
			while (true) {
				...
				// TODO : code de sortie du chat à "masquer" pour la version étudiant.
				if (ProtocoleChat.estUneCommande(message) && 
						ProtocoleChat.commandeDuMessage(message) == ProtocoleChat.COMMANDE_SORTIE_CHAT) {
					// Demande de sortie du chat par le client.
					break;
				}
			}
		}
	...
	private void traiteMessage(String message)
	{
		if (ProtocoleChat.estUneCommande(message)) {
			...
			switch (ProtocoleChat.commandeDuMessage(message)) {
				...
				// TODO : code de sortie du chat à "masquer" pour la version étudiant.
				case ProtocoleChat.COMMANDE_SORTIE_CHAT :
					this.chat.sortClientChat(this);
					break;
